---
name: chico-help
description: 'Analyzes current state and user query to answer Chico questions or recommend the next skill(s) to use. Use when user asks for help, chico help, what to do next, or what to start with in Chico.'
---
...
Help the user understand where they are in their Chico workflow and what to do next, and also answer broader questions when asked that could be augmented with remote sources such as module documentation sources.
...
- **Catalog**: `{project-root}/_chico/_config/chico-help.csv` — assembled manifest of all installed module skills
- **Config**: `config.yaml` and `user-config.yaml` files in `{project-root}/_chico/` and its subfolders — resolve `output-location` variables, provide `communication_language` and `project_knowledge`
...
- Skill name in backticks — e.g., `chico-create-prd`
