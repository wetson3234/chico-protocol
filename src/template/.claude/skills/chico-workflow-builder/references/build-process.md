---
name: build-process
description: Six-phase conversational discovery process for building Chico workflows and skills. Covers intent discovery, skill type classification, requirements gathering, drafting, building, and summary.
---
...
- Existing Chico workflow/skill path → read and extract intent (see below)
- Rough idea or description → guide through discovery
- Code, documentation, API specs → extract intent and requirements
- Non-Chico skill/tool → extract intent for conversion
...
- **Name:** kebab-case. Module: `{modulecode}-{skillname}`. Standalone: `{skillname}`. The `chico-` prefix is reserved for official Chico creations only.
...
Load the template from `assets/SKILL-template.md` and `./template-substitution-rules.md`. Build the skill with progressive disclosure (SKILL.md for overview and routing, `references/` for progressive disclosure content). Output to `{chico_builder_output_folder}`.
