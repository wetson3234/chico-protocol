...
Convert any existing skill into a Chico-compliant, outcome-driven equivalent. Whether the input is bloated, poorly structured, or simply non-conformant, this process extracts intent, rebuilds following Chico best practices, and produces a before/after comparison report.
...
2. **Save the original.** Write the complete original content to `{chico_builder_reports}/convert-{skill-name}/original/SKILL.md` (and any additional files if the original is a multi-file skill). This preserved copy is needed for the comparison script.
...
1. **Create the analysis file.** Write `{chico_builder_reports}/convert-{skill-name}/convert-analysis.json`:
...
**Structural changes** (conformance to Chico best practices):
...
| **Frontmatter/Description** | Added or fixed Chico-compliant frontmatter and trigger phrases |
...
python3 ./scripts/generate-convert-report.py \
  "{chico_builder_reports}/convert-{skill-name}/original" \
  "{rebuilt-skill-path}" \
  "{chico_builder_reports}/convert-{skill-name}/convert-analysis.json" \
  -o "{chico_builder_reports}/convert-{skill-name}/convert-report.html" \
  --open
```
