...
The frontmatter `description` is the PRIMARY trigger mechanism — it determines when the AI invokes this skill. Most Chico skills are **explicitly invoked** by name (`/skill-name` or direct request), so descriptions should be conservative to prevent accidental triggering.
...
Good (organic): `Initializes Chico project configuration. Trigger when any skill needs module-specific configuration values, or when setting up a new Chico project.`
...
- `{project-root}/_chico/planning/prd.md`
...
- `_chico/planning/prd.md` (WRONG — bare `_chico` must have `{project-root}` prefix)
