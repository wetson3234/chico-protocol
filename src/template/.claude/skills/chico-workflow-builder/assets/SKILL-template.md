...
{if-module}
Load available config from `{project-root}/_chico/config.yaml` and `{project-root}/_chico/config.user.yaml` (root level and `{module-code}` section). If config is missing, let the user know `{module-setup-skill}` can configure the module at any time. Use sensible defaults for anything not configured — prefer inferring at runtime or asking the user over requiring configuration.
{/if-module}
{if-standalone}
Load available config from `{project-root}/_chico/config.yaml` and `{project-root}/_chico/config.user.yaml` if present. Use sensible defaults for anything not configured.
{/if-standalone}
