...
# dependencies = []
# ///
"""Remove legacy module directories from _chico/ after config migration.
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Remove legacy module directories from _chico/ after config migration."
    )
    parser.add_argument(
        "--chico-dir",
        required=True,
        help="Path to the _chico/ directory",
    )
    parser.add_argument(
        "--module-code",
        required=True,
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Additional directory names under _chico/ to remove (repeatable)",
    )
    parser.add_argument(
        "--skills-dir",
...
def find_skill_dirs(base_path: str) -> list:
    ...
    Returns:
        List of skill directory names (e.g. ['chico-agent-builder', 'chico-builder-setup'])
    """
    skills = []
    root = Path(base_path)
    if not root.exists():
...
def verify_skills_installed(
    chico_dir: str, dirs_to_check: list, skills_dir: str, verbose: bool = False
) -> list:
    ...
    for dirname in dirs_to_check:
        legacy_path = Path(chico_dir) / dirname
...
def cleanup_directories(
    chico_dir: str, dirs_to_remove: list, verbose: bool = False
) -> tuple:
    """Remove specified directories under chico_dir.
...
def main():
    ...
    if args.skills_dir:
        ...
        verified_skills = verify_skills_installed(
            chico_dir, dirs_to_remove, args.skills_dir, args.verbose
        )

    # Remove directories
    removed, not_found, total_files = cleanup_directories(
        chico_dir, dirs_to_remove, args.verbose
    )

    # Build result
    result = {
        "status": "success",
        "chico_dir": str(Path(chico_dir).resolve()),
        "directories_removed": removed,
