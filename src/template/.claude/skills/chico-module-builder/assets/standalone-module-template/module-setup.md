...
- The module is not yet registered in `{project-root}/_chico/config.yaml`
...
2. Check if `{project-root}/_chico/config.yaml` exists — if a section matching the module's code is already present, inform the user this is an update (reconfiguration)
...
- `user_name` (default: Chico) — written exclusively to `config.user.yaml`
...
- `output_folder` (default: `{project-root}/_chico-output`) — written to `config.yaml` at root, shared across all modules
...
```bash
python3 ./scripts/merge-config.py --config-path "{project-root}/_chico/config.yaml" --user-config-path "{project-root}/_chico/config.user.yaml" --module-yaml ./assets/module.yaml --answers {temp-file}
python3 ./scripts/merge-help-csv.py --target "{project-root}/_chico/module-help.csv" --source ./assets/module-help.csv --module-code {module-code}
```
