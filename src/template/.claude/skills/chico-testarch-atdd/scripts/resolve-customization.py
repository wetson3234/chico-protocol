...
# ///
"""Resolve customization for a modular skill using three-layer TOML merge.

Reads customization from three layers (highest priority first):
  1. {project-root}/_chico/customizations/{name}.user.toml (personal, gitignored)
  2. {project-root}/_chico/customizations/{name}.toml (team/org, committed)
...
def find_project_root(start: Path) -> Path | None:
    """Walk up from *start* looking for a directory containing ``_chico/`` or ``.git``."""
    current = start.resolve()
    while True:
        if (current / "_chico").is_dir() or (current / ".git").exists():
            return current
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Resolve modular skill customization (three-layer TOML merge).",
        epilog=(
    ...
    parser.add_argument(
        "skill_name",
        help="Skill identifier (e.g. chico-agent-pm, chico-product-brief)",
    )
    parser.add_argument(
        "--key",
        action="append",
    ...
    if project_root is not None:
        customizations_dir = project_root / "_chico" / "customizations"
