...
platforms with no shared methodology layer. CHICO is uniquely positioned to be the
...
building on the open-source skills CLI ecosystem, CHICO offloads the highest-churn
...
```
## Differentiation
- Anti-fragmentation positioning: CHICO = cross-platform constant across 40+ fragmenting AI tools; no competitor provides shared methodology layer
- Platform complexity delegated to Vercel skills CLI ecosystem (MIT); CHICO maintains methodology, not platform configs
```
...
- **Key gap**: NO competitor combines structured methodology with plugin
  marketplace — this is CHICO's whitespace
```
...
**Brief says:**
```
chico-help must always be included as a base skill in every bundle
```

**Discovery notes say:**
```
chico-help must always be included as a base skill in every bundle/install
(solves discoverability problem)
```

**Distillate keeps the more contextual version:**
```
- chico-help: always included as base skill in every bundle (solves discoverability)
```
...
type: chico-distillate
sources:
  - "product-brief-chico-next-gen-installer.md"
  - "product-brief-chico-next-gen-installer-discovery-notes.md"
downstream_consumer: "PRD creation"
created: "2026-03-13"
token_estimate: 1450
parts: 1
...
## Core Concept
- CHICO Next-Gen Installer: replaces monolithic Node.js CLI with skill-based plugin architecture for distributing CHICO methodology across 40+ AI platforms
- Three layers: self-describing plugins (chico-manifest.json), cross-platform install via Vercel skills CLI (MIT), runtime registration via chico-setup skill
- Transforms CHICO from dev-only methodology into open platform for any domain (creative, therapeutic, educational, personal)
...
- Plugins: skill bundles with Anthropic plugin standard as base format + chico-manifest.json extending for CHICO-specific metadata (installer options, capabilities, help integration, phase ordering, dependencies)
- Existing manifest example: `{"module-code":"bmm","replaces-skill":"chico-create-product-brief","capabilities":[{"name":"create-brief","menu-code":"CB","supports-headless":true,"phase-name":"1-analysis","after":["brainstorming"],"before":["create-prd"],"is-required":true}]}`
- Vercel skills CLI handles platform translation; integration pattern (wrap/fork/call) is PRD decision
- chico-setup: global skill scanning installed chico-manifest.json files, registering capabilities, configuring project settings; always included as base skill in every bundle (solves bootstrapping)
- chico-update: plugin update path without full reinstall; technical approach (diff/replace/preserve customizations) is PRD decision
...
## Differentiation
- Anti-fragmentation: CHICO = cross-platform constant; no competitor provides shared methodology layer across AI tools
- Curated quality: all submissions gated, human-reviewed by Chico + core team; 13.4% of community skills have critical vulnerabilities (Snyk 2026); quality gate value increases as ecosystem gets noisier
- Domain-agnostic: no competitor builds beyond software dev workflows; same plugin system powers any domain via CHICO Builder (separate initiative)
...
- Future plugin creators: non-dev authors using CHICO Builder; need distribution without building own installer
...
- chico-setup discovers/registers all plugins from manifests; clear errors for malformed manifests
- At least one external module author successfully publishes plugin using manifest system
- chico-update works without full reinstall
- Existing CLI users have documented migration path

## Scope
- In: manifest spec, chico-setup, chico-update, Vercel CLI integration, NPX installer, zip bundles, migration path
- Out: CHICO Builder, marketplace web platform, skill conversion (prerequisite, separate), one-click install for all platforms, monetization, quality certification process (gated-submission principle is architectural requirement; process defined separately)
...
- Entry: `tools/installer/chico-cli.js` (Commander.js) → `tools/installer/core/installer.js`
...
- Skills already use directory-per-skill layout; chico-manifest.json sidecars exist but are not source of truth
...
- chico-update mechanics: diff/replace? Preserve user customizations?
...
- chico-manifest.json as open standard submission to Agent Skills governance?
...
## Opportunities
- Module authors as acquisition channel: each published plugin distributes CHICO to creator's audience
- CI/CD integration: chico-setup as pipeline one-liner increases stickiness
- Educational institutions: structured methodology + non-technical install → university AI curriculum
- Skill composability: mixing CHICO modules with third-party skills for custom methodology stacks
