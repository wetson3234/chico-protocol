---
name: chico-bmb-setup
description: Sets up Chico Builder module in a project. Use when the user requests to 'install bmb module', 'configure Chico Builder', or 'setup Chico Builder'.
---
...
Installs and configures a Chico module into a project. Module identity (name, code, version) comes from `./assets/module.yaml`. Collects user preferences and writes them to three files:

- **`{project-root}/_chico/config.yaml`** — shared project config: core settings at root (e.g. `output_folder`, `document_output_language`) plus a section per module with metadata and module-specific values. User-only keys (`user_name`, `communication_language`) are **never** written here.
- **`{project-root}/_chico/config.user.yaml`** — personal settings intended to be gitignored: `user_name`, `communication_language`, and any module variable marked `user_setting: true` in `./assets/module.yaml`. These values live exclusively here.
- **`{project-root}/_chico/module-help.csv`** — registers module capabilities for the help system.
...
2. Check if `{project-root}/_chico/config.yaml` exists — if a section matching the module's code is already present, inform the user this is an update
3. Check for per-module configuration at `{project-root}/_chico/cmb/config.yaml` and `{project-root}/_chico/core/config.yaml`. If either file exists:
   - If `{project-root}/_chico/config.yaml` does **not** yet have a section for this module: this is a **fresh install**. Inform the user that installer config was detected and values will be consolidated into the new format.
   - If `{project-root}/_chico/config.yaml` **already** has a section for this module: this is a **legacy migration**. Inform the user that legacy per-module config was found alongside existing config, and legacy values will be used as fallback defaults.
   - In both cases, per-module config files and directories will be cleaned up after setup.

If the user provides arguments (e.g. `accept all defaults`, `--headless`, or inline values like `user name is Chico, I speak Swahili`), map any provided values to config keys, use defaults for the rest, and skip interactive prompting. Still display the full confirmation summary at the end.
...
**Core config** (only if no core keys exist yet): `user_name` (default: Chico), `communication_language` and `document_output_language` (default: English — ask as a single language question, both keys get the same answer), `output_folder` (default: `{project-root}/_chico-output`). Of these, `user_name` and `communication_language` are written exclusively to `config.user.yaml`. The rest go to `config.yaml` at root and are shared across all modules.
...
```bash
python3 ./scripts/merge-config.py --config-path "{project-root}/_chico/config.yaml" --user-config-path "{project-root}/_chico/config.user.yaml" --module-yaml ./assets/module.yaml --answers {temp-file} --legacy-dir "{project-root}/_chico"
python3 ./scripts/merge-help-csv.py --target "{project-root}/_chico/module-help.csv" --source ./assets/module-help.csv --legacy-dir "{project-root}/_chico" --module-code bmb
```
...
After both merge scripts complete successfully, remove the installer's package directories. Skills and agents in these directories are already installed at `.claude/skills/` — the `_chico/` directory should only contain config files.

```bash
python3 ./scripts/cleanup-legacy.py --chico-dir "{project-root}/_chico" --module-code bmb --also-remove _config --skills-dir "{project-root}/.claude/skills"
```
