...
{if-module}
Load available config from `{project-root}/_chico/config.yaml` and `{project-root}/_chico/config.user.yaml` (root level and `{module-code}` section).
{/if-module}
{if-standalone}
Load available config from `{project-root}/_chico/config.yaml` and `{project-root}/_chico/config.user.yaml` if present.
{/if-standalone}
...
Sanctum location: `{project-root}/_chico/memory/{skillName}/`
