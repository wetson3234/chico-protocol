#!/usr/bin/env python3
"""Deterministic path standards scanner for modular skills.

Validates all .md and .json files against path conventions:
1. {project-root} for any project-scope path (not just _chico)
2. Bare _chico references must have {project-root} prefix
...
6. No absolute paths
7. Memory paths must use {project-root}/_chico/memory/{skillName}/
...
# Bare _chico without {project-root} prefix — match _chico at word boundary
# but not when preceded by {project-root}/
BARE_CHICO_RE = re.compile(r'(?<!\{project-root\}/)_chico[/\s]')
# Absolute paths
...
# Memory path pattern: should use {project-root}/_chico/memory/
MEMORY_PATH_RE = re.compile(r'_chico/memory/\S+')
VALID_MEMORY_PATH_RE = re.compile(r'\{project-root\}/_chico/memory/[\w-]+/')
...
def scan_file(filepath: Path, skip_fenced: bool = True) -> list[dict]:
    ...
    # Bare _chico check — more nuanced, need to avoid false positives
    # inside {project-root}/_chico which is correct
    for match in BARE_CHICO_RE.finditer(content):
        pos = match.start()
        if skip_fenced and is_in_fenced_block(content, pos):
            continue
        start = max(0, pos - 30)
        ...
        findings.append({
            ...
            'severity': 'high',
            'category': 'bare-chico',
            'title': 'Bare _chico reference without {project-root} prefix',
            'detail': line_content[:120],
            'action': '',
        })

    # Memory path check — memory paths should use {project-root}/_chico/memory/{skillName}/
    for match in MEMORY_PATH_RE.finditer(content):
        ...
        if '{project-root}/' not in before:
            ...
            findings.append({
                ...
                'title': 'Memory path missing {project-root} prefix — use {project-root}/_chico/memory/',
                'detail': line_content[:120],
                'action': '',
            })
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description='Scan modular skill for path standard violations',
    )
    parser.add_argument(
        'skill_path',
        type=Path,
