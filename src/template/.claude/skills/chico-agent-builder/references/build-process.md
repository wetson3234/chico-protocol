---
name: build-process
description: Six-phase conversational discovery process for building Chico agents. Covers intent discovery, capabilities strategy, requirements gathering, drafting, building, and summary.
---
...
**If external skills involved:** Suggest `chico-module-builder` to bundle agents + skills into a cohesive module.
...
- **Naming:** Standalone: `agent-{name}`. Module: `{modulecode}-agent-{name}`. The `chico-` prefix is reserved for official Chico creations only.
- **Activation modes:** Interactive only, or Interactive + Headless (schedule/cron for background tasks)
- **Memory architecture:** Agent memory at `{project-root}/_chico/memory/{skillName}/`
...
- Memory: `{project-root}/_chico/memory/{skillName}/`
...
Build the agent using templates from `./assets/` and rules from `./references/template-substitution-rules.md`. Output to `{chico_builder_output_folder}`.
